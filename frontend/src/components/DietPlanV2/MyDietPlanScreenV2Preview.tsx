...
import DailyCalorieIntakeV2 from "./DailyCalorieIntakeV2";
import CollapsibleMealV2 from "./CollapsibleMealV2";
...
const V2MealsList: React.FC<{ meals: DietV2Meal[] }> = ({ meals }) => {
  ...
  return (
    <View style={[{ height: height * 0.5 }]}>
      ...
      >
        ...
        {meals.map((meal, i) => (
          <CollapsibleMealV2 key={meal.id} meal={meal} index={i} />
        ))}
      </CustomScrollView>
    </View>
  );
...
const V2TipsList: React.FC<{ tips: string[] }> = ({ tips }) => {
  const { spacing, layout } = useStyles();
  const { height } = useWindowDimensions();
  ...
  <View style={tabBarStyles.container}>
    {tabs.map((tab) => {
      ...
      return (
        ...
        >
          <Text
            fontVariant={active ? "bold" : "medium"}
            fontSize={13}
...
const MyDietPlanScreenV2Preview = () => {
    ...
    () => [
      {
        label: "דגשים",
        value: "דגשים",
        content: <V2TipsList tips={DIET_V2_MOCK_TIPS} />,
      },
      {
        label: "הארוחות שלי",
        value: "הארוחות שלי",
        content: <V2MealsList meals={DIET_V2_MOCK_PLAN.meals} />,
      },
      {
        label: "תפריט חכם",
        value: "תפריט חכם",
  ...
  return (
    <View style={[layout.flex1, colors.background]}>
      <View style={[spacing.pdBottomBar, layout.flex1, { gap: 12 }]}>
        <V2Header targets={targets} consumed={consumedTotals} />
        <View style={[layout.flex1]}>
          <View style={spacing.pdHorizontalMd}>
            <V2TabBar tabs={tabs} selected={selectedTab} onSelect={setSelectedTab} />
          </View>
          <Animated.View
            key={selectedTab}
...
export default MyDietPlanScreenV2Preview;
